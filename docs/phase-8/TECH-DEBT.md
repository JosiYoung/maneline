...
| 04-04 | `expense-receipt-upload` | SPA + column wired end-to-end. `ExpenseForm` now has a file input (PDF/JPG/PNG/HEIC/WEBP, 25 MB cap) that runs the standard 3-step presign → R2 PUT → commit pipeline, then attaches the `r2_object_id` to `expenses.receipt_r2_object_id` on save (column existed unused since 00009). Worker extended: new `expense_receipt` kind in `ALLOWED_CONTENT_TYPES`; `handleUploadSign` + `handleUploadCommit` swap the ownership check to `do_i_have_access_to_animal` for this kind (widens to trainers with an active grant); commit inserts only `r2_objects` (no typed row); `handleUploadReadUrl` adds the `expense_receipt` branch that resolves animal via `expenses.receipt_r2_object_id`. Works for both owner + trainer ExpenseForm callers. 🟡 | Worker deploy — dev SPA hits `/api/uploads/sign` on the deployed Worker and gets `400 bad_kind` until the new `expense_receipt` enum lands. |
